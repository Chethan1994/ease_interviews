...
const OwlMascot = ({ isPasswordFocused, mousePosition }: { isPasswordFocused: boolean, mousePosition: { x: number, y: number } }) => {
    // Calculate look direction limited to a range for the pupils
    // We dampen the movement (divide by 15) and clamp it so pupils stay in eyeballs
    const xLimit = 12;
    const yLimit = 12;
        ...
        <div className="w-64 h-64 relative transition-all duration-300 ease-out">
            <svg viewBox="0 0 200 200" className="w-full h-full drop-shadow-2xl">
                ...
                <g filter="url(#owl-shadow)">
                    {/* Main Body Shape */}
                    <path 
                        d="M 50 180 L 150 180 C 170 180 180 160 180 130 C 180 60 160 30 100 30 C 40 30 20 60 20 130 C 20 160 30 180 50 180 Z" 
                    ...
                    {/* Belly Patch */}
                    <path 
                        d="M 60 180 L 140 180 C 150 180 150 170 150 140 C 150 90 130 80 100 80 C 70 80 50 90 50 140 C 50 170 50 180 60 180 Z" 
                        fill="#ffffff"
                    />
                </g>

                {/* --- Head Features (Moves slightly with mouse) --- */}
                <g transform={`translate(${lookX * 0.2}, ${lookY * 0.2})`}>
                    ...
                    <g>
                        ...
                        {/* Pupils - Move based on mouse, but hidden if password focused */}
                        <g style={{ 
                ...
                {/* --- Wings / Hands (Animation Logic) --- */}
                {/* 
                   When password is focused, wings move up to cover eyes.
                   Origin is set to bottom corners so they rotate up.
                */}
                ...
                <g style={{
                    ...
                    transform: isPasswordFocused 
                        ? 'translate(20px, -50px) rotate(160deg)' 
                        : 'translate(0, 0) rotate(0deg)',
                    transition: 'transform 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)'
                }}>
                    <path 
                        d="M 20 130 Q 5 160 40 170 Q 50 140 20 130" 
                        fill="#cbd5e1" 
                        stroke="#94a3b8" 
                        strokeWidth="2" 
                    />
                 ...
                 <g style={{
                    ...
                    <path 
                        d="M 180 130 Q 195 160 160 170 Q 150 140 180 130" 
                        fill="#cbd5e1" 
                        stroke="#94a3b8" 
                        strokeWidth="2" 
                    />
...
export const AuthPage: React.FC<AuthPageProps> = ({ onSuccess }) => {
    ...
    // Handle initial mode based on navigation state
    useEffect(() => {
        // Clear error when switching modes via navigation
        setError('');
    ...
    const toggleAuthMode = () => {
        setIsLogin(!isLogin);
        setError(''); // CRITICAL FIX: Clear error when switching modes manually
    };
